...
class SectionDoubleP:
    ...
    def push_back(self, Name=".NewSec", VirtualSize=0x00000000, VirtualAddress=0x00000000, 
        ...
        if self.pe.FILE_HEADER.NumberOfSections == len(self.pe.sections):
            ...
            print self.pe.OPTIONAL_HEADER.SizeOfHeaders < section_table_offset + (self.pe.FILE_HEADER.NumberOfSections+1)*0x28
            print self.pe.get_data(section_table_offset + (self.pe.FILE_HEADER.NumberOfSections)*0x28, 0x28)
            
            if ( self.pe.OPTIONAL_HEADER.SizeOfHeaders < section_table_offset + (self.pe.FILE_HEADER.NumberOfSections+1)*0x28
                or not  self.__is_null_data(self.pe.get_data(section_table_offset + (self.pe.FILE_HEADER.NumberOfSections)*0x28, 0x28))):
                
                # Checking if more space can be added.
                #if self.pe.OPTIONAL_HEADER.SizeOfHeaders < self.pe.sections[0].VirtualAddress:
                if (section_table_offset + (self.pe.FILE_HEADER.NumberOfSections * 0x28)) < self.pe.OPTIONAL_HEADER.SizeOfHeaders:
...
def main(argv):
    ...
    # 0x00400000.
    data="\xE9\xDA\xF4\xFC\xFF\x90\xE9\xDA\xF4\xFC\xFF\x90\xE9\xDA\xF4\xFC\xFF\x90\xE9\xDA\xF4\xFC\xFF\x90\xE9\xDA\xF4\xFC\xFF\x90"
    ...
    pe.write(filename="calc_n0p.exe")
